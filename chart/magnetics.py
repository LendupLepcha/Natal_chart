...
def get_magnetic_data(latitude, longitude, year, month, day, hour, minute,  entry_time, name, e_datetime):
    ...
    x = []
    for i in x_axis:
        x.append(datetime.strptime(str(i), '%Y-%m-%d %H:%M:00'))
    y = []
...
#     st.set_y(1.01)
    for i in range(4):
        ...
        ax[i].plot(datetime.strptime(datass[0], '%Y-%m-%d %H:%M:00'), datass[i+1], 'ro');
        ax[i].text(datetime.strptime(datass[0], '%Y-%m-%d %H:%M:00'), datass[i+1], " - "+labels[i]+"="+str(datass[i+1]))
